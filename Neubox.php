    ...
    class Neubox extends RegistrarModule {
		...
		public function config_fields($data=[]):array {
				...
				return [
						'apikey'          => [
								'name'              => "Api Key",
								'description'       => "Neubox API Key field",
								'type'              => "text",
								'value'             => $data["apikey"] ?? '',
								'placeholder'       => "xxxxxxxxxxxxxxxxxxxxxx-xxxx",
						],
						'privatekey'          => [
								'type'              => "password",
								'name'              => "Private Key",
								'description'       => "Neubox Private API Key field",
		...
		// Required function
		// NOTE: La transferencia se realiza de forma manual luego de que el cliente hace la solicitud y provee el codigo EPP
		...
		public function domains(){
			...
			$req = new NeuboxPetitions($apiAuthData);
			try {
				$json_domains = $req->getDomains();
			} catch(\Exception $e) {
				$this->error = "An error has occured trying to fetch your domains, try again later."; # or $this->lang["error-message-variable"];
				return false;
			}

			$domains = json_decode($json_domains);

			$result     = [];

			if($domains->datos->result == 'success') {
					foreach($domains->datos->response AS $domain_data) {
							$cdate      = $domain_data->registrationdate;
							$edate      = $domain_data->expirydate;
